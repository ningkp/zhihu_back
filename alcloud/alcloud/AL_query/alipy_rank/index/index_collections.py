...
def map_array_index(whole_array, sub_array):
    '''For each element in sub_array, return the index of it in whole_array'''
    tr_ob = []
    for entry in sub_array:
        assert entry in whole_array
        tr_ob.append(np.argwhere(whole_array == entry)[0][0])
    return tr_ob
