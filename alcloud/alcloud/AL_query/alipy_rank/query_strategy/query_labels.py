...
class QueryInstanceUncertainty(BaseIndexQuery):
    ...
    @classmethod
    def rank_unlabeled_data_with_default_model(cls, unlabel_index, unlab_prob_prediction, measure='entropy',  **kwargs):
        '''
        need_feature_mat: no,
        need_label: no,
        need_prediction: yes,
        '''
        unlabel_index = np.asarray(unlabel_index)
        pv = np.asarray(unlab_prob_prediction)
        assert len(unlabel_index) == len(unlab_prob_prediction)
        spv = np.shape(unlab_prob_prediction)  # shape of predict value

        if measure == 'distance_to_boundary':
            assert (len(spv) in [1, 2])
            if len(spv) == 2:
                if spv[1] != 1:
                    raise Exception('1d or 2d with 1 column array is expected, but received: \n%s' % str(pv))
                ...
                pv = np.absolute(pv)    # 0 is the decision boundary
            sorted_ind = np.argsort(pv)
            return unlabel_index[sorted_ind]
        ...
        if measure == 'entropy':
            ...
            entro = [-np.sum(vec * np.log(vec)) for vec in pv]
            assert (len(np.shape(entro)) == 1)
            return unlabel_index[np.argsort(entro)[::-1]]

        if measure == 'margin':
            # calc margin
            pat = np.partition(pv, (spv[1] - 2, spv[1] - 1), axis=1)
            pat = pat[:, spv[1] - 2] - pat[:, spv[1] - 1]
            return unlabel_index[np.argsort(pat)[::-1]]
    ...
    @classmethod
    def rank_unlabeled_data(cls, unlabel_index, unlab_prob_prediction, measure='entropy', **kwargs):
        return cls.rank_unlabeled_data_with_default_model(unlabel_index, unlab_prob_prediction, measure=measure, **kwargs)
...
class QueryRandom(BaseIndexQuery):
    ...
    @classmethod
    def rank_unlabeled_data_with_default_model(cls, unlabel_index, **kwargs):
        rp = randperm(len(unlabel_index)-1)
        return np.asarray(unlabel_index)[rp]

    @classmethod
    def rank_unlabeled_data(cls, unlabel_index, **kwargs):
        rp = randperm(len(unlabel_index) - 1)
        return np.asarray(unlabel_index)[rp]
...
class QueryInstanceQBC(BaseIndexQuery):
    ...
    @classmethod
    def rank_unlabeled_data_with_default_model(cls, X, y, label_index, unlabel_index, disagreement='vote_entropy', **kwargs):
        '''
        need_feature_mat: yes,
        need_label: yes,
        need_prediction: no,
        '''
        X = np.asarray(X)
        ...
        if disagreement == 'vote_entropy':
            score = cls.calc_vote_entropy([estimator.predict(unlabel_x) for estimator in est_arr])
        else:
            score = cls.calc_avg_KL_divergence([estimator.predict_proba(unlabel_x) for estimator in est_arr])
        return unlabel_index[np.argsort(score)[::-1]]

    @classmethod
    def rank_unlabeled_data(cls, unlabel_index, unlab_prob_prediction, disagreement='vote_entropy', **kwargs):
        if disagreement == 'vote_entropy':
            score = cls.calc_vote_entropy(unlab_prob_prediction)
        else:
            score = cls.calc_avg_KL_divergence(unlab_prob_prediction)
        return np.asarray(unlabel_index)[np.argsort(score)[::-1]]   # descent
...
class QueryExpectedErrorReduction(BaseIndexQuery):
    ...
    @classmethod
    def rank_unlabeled_data_with_default_model(cls, X, y, label_index, unlabel_index, **kwargs):
        '''
        need_feature_mat: yes,
        need_label: yes,
        need_prediction: no,
        ...
        if X is None or y is None:
            raise Exception('Data matrix is not provided.')
        model = LogisticRegression(solver='liblinear')
        model.fit(X[label_index if isinstance(label_index, (list, np.ndarray)) else label_index.index],
                  y[label_index if isinstance(label_index, (list, np.ndarray)) else label_index.index])
        ...
        for i in range(spv[0]):
            new_train_inds = np.append(label_index, unlabel_index[i])
            new_train_X = X[new_train_inds, :]
            unlabel_ind = list(unlabel_index)
            unlabel_ind.pop(i)
            new_unlabel_X = X[unlabel_ind, :]
            score = []
            for yi in classes:
                new_model = copy.deepcopy(model)
                new_model.fit(new_train_X, np.append(label_y, yi))
                prob = new_model.predict_proba(new_unlabel_X)
                score.append(pv[i, yi] * cls.log_loss(prob))
            scores.append(np.sum(score))

        return np.asarray(unlabel_index)[np.argsort(scores)]

    @classmethod
    def rank_unlabeled_data(cls, X, y, label_index, unlabel_index, model, **kwargs):
        if X is None or y is None:
            raise Exception('Data matrix is not provided.')
        model = model
        # model.fit(X[label_index if isinstance(label_index, (list, np.ndarray)) else label_index.index],
...
class QueryInstanceQUIRE(BaseIndexQuery):
    ...
    def rank_unlabeled_data_with_default_model(cls, X, y, label_index, unlabel_index, **kwargs):
        ...
        need_prediction: no,
        '''
        unlab_data_score = dict()

        lmbda = kwargs.pop('lambda', 1.)
        ...
        for i, each_index in enumerate(Uindex):
            ...
            #     query_index = each_index
            #     min_eva = eva
            unlab_data_score[each_index] = eva

        # a sorted list of tuples  [(3458, 0.1), (1422, 0.2), (2456, 0.3), (1348, 0.8)]
        sorted_data = sorted(unlab_data_score.items(), key=lambda d: d[1], reverse=False)
        return np.asarray([tup[0] for tup in sorted_data])

    @classmethod
    def rank_unlabeled_data(cls, X, y, label_index, unlabel_index, **kwargs):
        return cls.rank_unlabeled_data_with_default_model(X, y, label_index, unlabel_index, **kwargs)
...
class QueryInstanceBMDR(BaseIndexQuery):
    ...
    def __init__(self, X, y, beta=1, gamma=0.1, rho=1, **kwargs):
        try:
            import cvxpy
            self._cvxpy = cvxpy
        except:
    ...
    def select(self, label_index, unlabel_index, batch_size=5, qp_solver='ECOS', **kwargs):
        ...
        unlabel_index = np.asarray(unlabel_index)
        label_index = np.asarray(label_index)
        batch_size = int(len(unlabel_index)//4)
        ...
        while 1:
            ...
            # dr_largest = np.argsort(dr_weight)[::-1]
            ...
            if set(last_round_selected) == set(select_ind) or iter_round > 15:
                return np.asarray(unlabel_index)[np.argsort(dr_weight)[::-1]]
            else:
    ...
    def rank_unlabeled_data_with_default_model(cls, X, y, label_index, unlabel_index, **kwargs):
        ...
        '''
        bmdr = cls(X, y, **kwargs)
        return bmdr.select(label_index, unlabel_index)
...
class QueryInstanceSPAL(BaseIndexQuery):
    ...
    def select(self, label_index, unlabel_index, batch_size=5, qp_solver='ECOS', **kwargs):
        ...
        while 1:
            ...
            select_ind = np.asarray(unlabel_index)[dr_largest]
            if set(last_round_selected) == set(select_ind) or iter_round > 15:
                return np.asarray(unlabel_index)[np.argsort(dr_weight * es_weight)[::-1]]
            else:
                last_round_selected = copy.copy(select_ind)
                # print(dr_largest)
                # print(dr_weight[dr_largest])
    ...
    def rank_unlabeled_data_with_default_model(cls, X, y, label_index, unlabel_index, **kwargs):
        ...
        '''
        spal = cls(X, y, **kwargs)
        return spal.select(label_index, unlabel_index)
