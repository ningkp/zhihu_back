...
from ..index.index_collections import map_array_index
...
class QueryMultiLabelQUIRE(BaseMultiLabelQuery):
    ...
    def select(self, label_index, unlabel_index, **kwargs):
        ...
        sort_val = np.argsort(vals)  # ascent
        idx_ondim = np.asarray(Uidx)[sort_val]
        return [(one_id % self.X.shape[0], one_id // self.X.shape[0]) for one_id in idx_ondim]
    ...
    def rank_unlabeled_data_with_default_model(cls, X, y, label_index, unlabel_index, **kwargs):
        ...
        need_prediction: no,
        '''
        quire = cls(X, y, **kwargs)
        return quire.select(label_index, unlabel_index)
...
class QueryMultiLabelAUDI(BaseMultiLabelQuery):
    ...
    def select(self, label_index, unlabel_index, model=None, epsilon=0.5, **kwargs):
        ...
        insvals = -np.abs(
            (np.sum(labels == 1, axis=1) - avgP) / np.fmax(np.sum(W[data_ind, :] == 1, axis=1), epsilon))
        ...
        if sel_ins_label_mask[selected_lab]:
            ...
            argsorted_dis = np.argsort(dis)  # descent
    ...
    @classmethod
    def rank_unlabeled_data_with_default_model(cls, y, label_index, unlabel_index, unlab_prob_prediction, **kwargs):
        '''
        need_feature_mat: no,
        need_label: yes,
        need_prediction: yes,
        ...
        W = unlabel_index.get_matrix_mask(mat_shape=y.shape, fill_value=1, sparse=False)
        data_ind = np.nonzero(np.sum(W, axis=1) > 0)[0]
        data_ind_ini_bk = data_ind.copy()
        # select instance by LCI
        pres = unlab_prob_prediction
        n_samples, n_class = np.shape(pres)
        labels = -np.ones((n_samples, n_class))
        for line in range(n_class - 1):
            gt = np.nonzero(pres[:, line] > pres[:, n_class - 1])[0]
            labels[gt, line] = 1
        avgP = np.mean(np.sum(y[label_index.get_unbroken_instances(), :] == 1, axis=1))

        ranked_ind = []
        while len(unlabel_index) > 0:
            ind_in_unlab = map_array_index(whole_array=data_ind_ini_bk, sub_array=data_ind)
            labels_tp = labels[ind_in_unlab]
            pres_tp = pres[ind_in_unlab]
            insvals = -np.abs(
                (np.sum(labels_tp == 1, axis=1) - avgP) / np.fmax(np.sum(W[data_ind, :] == 1, axis=1), epsilon))
            selected_ins = np.argmin(insvals)
            # last line in pres is the predict value of dummy label
            # select label by calculating the distance between each label with dummy label
            ...
            # set the known entries to inf
            pres_mask = np.asarray(1 - W[data_ind], dtype=bool)
            pres_tmp = pres_tp[:, 0:-1]
            pres_tmp[pres_mask] = np.NINF
            pres_tp[:, 0:-1] = pres_tmp
            sel_ins_label_mask = pres_mask[selected_ins]

            dis = np.abs(pres_tp[selected_ins, 0:-1] - pres_tp[selected_ins, -1])
            selected_lab = np.argmin(dis)
            if sel_ins_label_mask[selected_lab]:
                # select a labeled entry
                argsorted_dis = np.argsort(dis)  # descent
                for dis_ind in argsorted_dis:
                    if not sel_ins_label_mask[dis_ind]:
                        ...
                        break
            selected_ins = data_ind[selected_ins]
            ranked_ind.append((selected_ins, selected_lab))

            unlabel_index.difference_update([(selected_ins, selected_lab)])
            W = unlabel_index.get_matrix_mask(mat_shape=y.shape, fill_value=1, sparse=False)
            data_ind = np.nonzero(np.sum(W, axis=1) > 0)[0]
    ...
    @classmethod
    def rank_unlabeled_data(cls, y, label_index, unlabel_index, unlab_prob_prediction, **kwargs):
        return cls.rank_unlabeled_data_with_default_model(y, label_index, unlabel_index, unlab_prob_prediction,
                                                          **kwargs)
...
class QueryMultiLabelMMC(BaseIndexQuery):
    ...
    def rank_unlabeled_data_with_default_model(cls, X, y, label_index, unlabel_index, **kwargs):
        ...
        if isinstance(unlabel_index, (list, np.ndarray)):
            ...
            raise TypeError("index type error")
        mmc = cls(X, y, **kwargs)
        return mmc.select(label_index, unlabel_index, batch_size=len(unlabel_index))
    ...
    def rank_unlabeled_data(cls, X, y, label_index, unlabel_index, model, **kwargs):
        if isinstance(unlabel_index, (list, np.ndarray)):
            ...
            raise TypeError("index type error")
        mmc = cls(X, y, br_base=model, **kwargs)
        return mmc.select(label_index, unlabel_index, batch_size=len(unlabel_index))
...
class QueryMultiLabelAdaptive(BaseIndexQuery):
    ...
    def rank_unlabeled_data_with_default_model(cls, X, y, label_index, unlabel_index, **kwargs):
        ...
        if isinstance(unlabel_index, (list, np.ndarray)):
            ...
            raise TypeError("index type error")
        adaptive = cls(X, y, **kwargs)
        return adaptive.select(label_index, unlabel_index, batch_size=len(unlabel_index))
    ...
    def rank_unlabeled_data(cls, X, y, label_index, unlabel_index, model, **kwargs):
        if isinstance(unlabel_index, (list, np.ndarray)):
            ...
            raise TypeError("index type error")
        adaptive = cls(X, y, base_clf=model, **kwargs)
        return adaptive.select(label_index, unlabel_index, batch_size=len(unlabel_index))
...
class QueryMultiLabelRandom(BaseMultiLabelQuery):
    ...
    def rank_unlabeled_data_with_default_model(cls, unlabel_index, **kwargs):
        ...
        need_label: no,
        need_prediction: no'''
        select_type = kwargs.pop('select_type', 'ins')
        if select_type == 'ins':
            unkonwn_entries = cls()._check_multi_label_ind(unlabel_index)
            unkonwn_ins = unkonwn_entries.get_instance_index()
            perm = randperm(len(unkonwn_ins) - 1)
            return [(unkonwn_ins[i],) for i in perm]
        elif select_type == 'ins-lab':
            unkonwn_entries = cls()._check_multi_label_ind(unlabel_index)
            perm = randperm(len(unkonwn_entries) - 1)
            tpl = list(unkonwn_entries.index)
            return [tpl[i] for i in perm]
        else:
            raise ValueError("select_type must be one of {'ins', 'ins-lab'}")
    ...
    def rank_unlabeled_data(cls, unlabel_index, **kwargs):
        return cls.rank_unlabeled_data_with_default_model(unlabel_index, **kwargs)
