...
class QueryTypeAURO(BaseMultiLabelQuery):
    ...
    def select(self, label_index, unlabel_index, model=None, y_mat=None, **kwargs):
        ...
        while len(np.nonzero(np.sum(unlab_mask, axis=1) > 1)[0]) > 0:
            ...
            sort_dis = np.argsort(dis)  # descent
            ...
            # add result
            data_rank.append([unlab_ins_ind[selected_ins], y1, y2])
            # update mask
            unlab_mask[selected_ins, y1] = 0
            unlab_mask[selected_ins, y2] = 0

        return data_rank

    @classmethod
    def rank_unlabeled_data_with_default_model(cls, unlabel_index, unlab_prob_prediction, y_shape, **kwargs):
        '''
        need_feature_mat: no,
        need_label: yes,
        need_prediction: yes,

        need_label_shape: yes

        Parameters:
        -----------
        label_index: IndexCollection
        unlabel_index: IndexCollection
        unlab_prob_prediction: 2D array, with n_data rows and n_label+1 cols predicted by a labelranking model
        y_shape: tuple, (row, col)
        '''

        # select instance with least queries
        W = unlabel_index.get_matrix_mask(mat_shape=y_shape, fill_value=1, sparse=False)
        unlab_ins_ind = np.nonzero(np.sum(W, axis=1) > 1)[0]
        unlab_mask = W[unlab_ins_ind]
        ...
        while len(np.nonzero(np.sum(unlab_mask, axis=1) > 1)[0]) > 0:
            ...
            # set the known entries to -inf
            pres_mask = np.asarray(1 - unlab_mask, dtype=bool)
            pres_tmp = unlab_prob_prediction[:, 0:-1]
            pres_tmp[pres_mask] = np.NINF
            unlab_prob_prediction[:, 0:-1] = pres_tmp
            sel_ins_label_mask = pres_mask[selected_ins]

            # last line in pres is the predict value of dummy label
            # select label by calculating the distance between each label with dummy label
            y1 = np.argmax(unlab_prob_prediction[selected_ins, 0:-1])
            dis = np.abs(unlab_prob_prediction[selected_ins, 0:-1] - unlab_prob_prediction[selected_ins, -1])
            sort_dis = np.argsort(dis)  # descent
            for dis_ind in sort_dis:
                if not sel_ins_label_mask[dis_ind] and dis_ind != y1:
                    y2 = dis_ind
    ...
    @classmethod
    def rank_unlabeled_data(cls, unlabel_index, unlab_prob_prediction, y_shape, **kwargs):
        return cls.rank_unlabeled_data_with_default_model(unlabel_index, unlab_prob_prediction, y_shape, **kwargs)
