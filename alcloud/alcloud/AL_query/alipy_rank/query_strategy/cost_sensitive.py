...
class QueryCostSensitiveHALC(BaseMultiLabelQuery):
    ...
    @classmethod
    def rank_unlabeled_data(cls, X, y, label_index, unlabel_index, budget, label_tree, hierarchy_costs, model, **kwargs):
        """
        label_index: {list, np.ndarray, MultiLabelIndexCollection}
            The indexes of labeled samples. It should be a 1d array of indexes (column major, start from 0) or
            MultiLabelIndexCollection or a list of tuples with 2 elements, in which,
        ...
        halc = cls(X=X, y=y, label_tree=label_tree)
        return halc.select(label_index=label_index, unlabel_index=unlabel_index, cost=hierarchy_costs, 
                            budget=budget, base_model=model)

    @classmethod
    def rank_unlabeled_data_with_default_model(cls, X, y, label_index, unlabel_index, budget, label_tree, hierarchy_costs, **kwargs):
        cls.rank_unlabeled_data(X, y, label_index, unlabel_index, budget, label_tree, hierarchy_costs, model=SVC())
...
class QueryCostSensitiveRandom(BaseMultiLabelQuery):
    ...
    @classmethod
    def rank_unlabeled_data(cls, label_index, unlabel_index, budget, hierarchy_costs, **kwargs):
        cs_rand = cls()
        return cs_rand.select(label_index=label_index, unlabel_index=unlabel_index, cost=hierarchy_costs, budget=budget)

    @classmethod
    def rank_unlabeled_data_with_default_model(cls, **kwargs):
        cls.rank_unlabeled_data(**kwargs)
...
class QueryCostSensitivePerformance(BaseMultiLabelQuery):
    ...
    @classmethod
    def rank_unlabeled_data(cls, X, y, label_index, unlabel_index, budget, hierarchy_costs, model, **kwargs):
        if model is None:
            model=SVC()

        cs_unc = cls(X=X, y=y)
        return cs_unc.select(label_index=label_index, unlabel_index=unlabel_index, 
                                cost=hierarchy_costs, budget=budget, basemodel=model)

    @classmethod
    def rank_unlabeled_data_with_default_model(cls, X, y, label_index, unlabel_index, budget, hierarchy_costs, **kwargs):
        cls.rank_unlabeled_data(X, y, label_index, unlabel_index, budget, hierarchy_costs, model=SVC())
