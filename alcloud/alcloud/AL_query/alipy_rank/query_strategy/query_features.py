...
class QueryFeatureRandom(BaseFeatureQuery):
    ...
    def rank_unlabeled_data_with_default_model(cls, unlabel_index, **kwargs):
        ...
        need_prediction: no,
        '''
        unkonwn_entries = cls()._check_feature_ind(unlabel_index)
...
class QueryFeatureStability(BaseFeatureQuery):
    ...
    def select_by_mask(self, observed_mask, **kwargs):
        ...
        mean_mat = np.zeros(mc_sh)
        assert len(X_mc_arr) >= 2

        var_mat = np.zeros(mc_sh)
        for i in range(mc_sh[0]):
            for j in range(mc_sh[1]):
                var_mat[i, j] = np.var([mat[i][j] for mat in X_mc_arr])
                mean_mat[i, j] = np.mean([mat[i][j] for mat in X_mc_arr])
        var_mat *= 1 - observed_mask
        selected_feature = np.argsort(var_mat.flatten(order='F'))

        return [(one_feature // mc_sh[1], one_feature % mc_sh[1]) for one_feature in selected_feature], mean_mat

    @classmethod
    def rank_unlabeled_data_with_default_model(cls, X, label_index, unlabel_index, **kwargs):
        '''
        need_feature_mat: yes,
        need_label: no,
        need_prediction: no,
        '''
        stab = cls(X, None, list(range(len(label_index)+len(unlabel_index))), **kwargs)
        return stab.select(label_index, unlabel_index)
